...
//1234, 2345, 3456 only possible options
// 1,3,4,5,5
class SmallStraight extends Rule{
  evalRoll = dice =>{
    const d = new Set(dice);
    const freqs = this.freq(dice)
    return d.size>=4 && !(freqs.includes(3) && freqs.includes(4) && freqs.includes(5) ) ? this.score : 0;
  }
}
...
class LargeStraight extends Rule {
  evalRoll = dice => {
    ...
    return d.size === 5 && (!d.has(1) || !d.has(6)) ? this.score : 0; // d.size === 5 && !(d.has(1) && d.has(6))
  };
}
// && || 
// !var1 || !var2 === !(var1 && var2)
